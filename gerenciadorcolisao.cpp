...
{
    for (int i = 0; i < _listaPersonagens->get_tamanhoLista(); i++) {                               // percorre a lista de personagens verificando com cada obstaculo se ha colisao
        ...
        for (int j = 0; j < _listaObstaculos->get_tamanhoLista(); j++) {
            ...
            if (distancia.x < 0.0f && distancia.y < 0.0f) {
                obstaculo->colisao(personagem, distancia);                                          // se houver colisao chama o metodo polimorfico
            }
        }
    }
}
...
// funcao que executa as colisoes
